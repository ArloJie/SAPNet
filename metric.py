...
def normalize_pil(pre, gt):
    ...
    # Normalize GT to 0/1 binary mask
    gt = gt / (gt.max() + 1e-8)
    ...
    if pre.max() > 1:
        ...
        pre = np.clip(pre, 0, 1)  # in case of small float errors
...
#     def update(self, pred, gt):
#         """
#         pred: numpy array [H, W], float, range [0, 1] or [0, 255]
#         gt:   numpy array [H, W], binary {0, 1}
#         """
#         if pred.max() > 1:
#             pred = pred / 255.0
#         gt = (gt >= 0.5)
#
#         thresholds = np.linspace(0, 1, 256, endpoint=True)[..., None, None]  # shape: [256, 1, 1]
#         bin_preds = (pred[None, :, :] >= thresholds)  # shape: [256, H, W]
#
#         em_list = np.array([self._emeasure_fast(bin_pred, gt) for bin_pred in bin_preds])
#         self.em_matrix.append(em_list)
#
#     def _emeasure_fast(self, FM, GT):
...
#             align_Matrix = self._alignment_term(dFM, dGT)
#             enhanced = self._enhanced_alignment(align_Matrix)
#         return np.mean(enhanced)
#
#     def _alignment_term(self, dFM, dGT):
#         mu_FM = dFM.mean()
#         mu_GT = dGT.mean()
#         align_FM = dFM - mu_FM
#         align_GT = dGT - mu_GT
#         denom = align_GT ** 2 + align_FM ** 2 + 1e-8
#         return 2 * (align_GT * align_FM) / denom
#
#     def _enhanced_alignment(self, align_Matrix):
#         return ((align_Matrix + 1) ** 2) / 4
#
#     def show(self):
#         em_array = np.array(self.em_matrix)  # shape: [N, 256]
#         em_curve = em_array.mean(axis=0)
#         meanEm = em_curve.mean()
#         maxEm = em_curve.max()
#         return meanEm, maxEm
